...
public class MatcherAssertIgnoringPatternTest {
    ...
    public void ignoresAnyMatchingFieldNames() {
        ParentBean expected = parent().childBean("value", 123).addToChildBeanList(child().childString("child").build
                ()).build();
        ParentBean actual = parent().childBean("value", 321).addToChildBeanList(child().childString("dlihc").build())
                .build();
